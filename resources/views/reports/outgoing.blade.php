...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-xs text-slate-500 mt-0.5">Rekapitulasi pengeluaran barang inventory untuk kebutuhan operasional workshop/proyek</p>
    </div>

    <div class="no-print flex items-center gap-2">
        <a href="{{ route('reports.outgoing.export', request()->all()) }}" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs shadow-md shadow-emerald-600/20 transition-all">
...
<div class="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-xs mb-6 no-print">
    <form action="{{ route('reports.outgoing') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
        <div>
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
    <div class="p-6 hidden print:block text-black text-center border-b border-slate-200">
        ...
        <p class="text-xs font-bold mt-1">LAPORAN BARANG KELUAR WAREHOUSE</p>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 text-xs font-medium">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td class="py-3.5 px-4 text-center font-bold text-rose-600 text-sm">-{{ number_format($row->quantity) }} {{ $row->item->unit->name }}</td>
                    <td class="py-3.5 px-4 font-semibold text-slate-800">{{ $row->destination }}</td>
                    <td class="py-3.5 px-4 text-slate-700">{{ $row->requester }}</td>
                    <td class="py-3.5 px-4 text-slate-500">{{ $row->user->name }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="8" class="py-12 text-center text-slate-400">Tidak ada data transaksi barang keluar.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
